...
class ShowAllOwnReservationsComponent extends Component {
    constructor(props) {
        super(props);
        const {t} = this.props;
        document.title = t("allReservations.pageTitle");
    ...
    componentDidMount() {
        this.sendOwnReservationsRequest();
    }
    ...
    sendOwnReservationsRequest = () => {
        const {t} = this.props;
        getFetch(`/app/reservation/own?canceled=` + this.state.getCanceled + "&past=" + this.state.getPast)
            .then(response => {
                if (response.ok) {
                    return response;
    ...
    sendPutCancelRequest = () => {
        ...
        putFetch('/app/reservation/cancel-own-reservation', undefined, header)
            .then((response) => {
                if (response.ok) {
    ...
    render() {
        ...
        if (!this.state.loading) {
            return (
                <Card>
                    <CardHeader
                        name={"showallReservationsHeader"}
                        title={tableHeader()}
...
export default withTranslation()(withRouter(ShowAllOwnReservationsComponent));
