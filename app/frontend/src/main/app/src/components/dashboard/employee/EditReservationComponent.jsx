...
class EditReservationComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            alleyName: "",
    ...
    getReservationData = () => {
        const {t} = this.props;
        getFetch('/app/reservation/get-reservation?reservationNumber=' + this.state.reservationNumber)
            .then((response) => {
                if (response.ok) {
                    return response;
            ...
            .then(data => {
                this.setState({
                    ...
                    weaponModelName: data.weapon.weaponModelName,
                    weaponSerialNumber: data.weapon.serialNumber,
                    ...
                    this.getConflictReservations();
                    this.getWeaponSerialNumbers(data.weapon.weaponModelName);
                });
            }).catch(() => {
    ...
    sendRequest = () => {
        ...
        const body = {
            ...
            "weaponSerialNumber": this.state.weaponSerialNumber,
        ...
        putFetch(
            '/app/reservation/update-reservation', body, header)
            .then((response) => {
                    if (response.ok) {
    ...
    getConflictReservations = () => {
        if (this.state.alleyName === "" || this.state.weaponSerialNumber === "" || this.state.reservationDate === "") {
            return;
        }
        const {t} = this.props;
        getFetch("/app/reservation/get-conflict-reservations-by-weapon?date=" + this.state.reservationDate +
            "T00:00&alleyName=" + this.state.alleyName + "&weaponSerialNumber=" + this.state.weaponSerialNumber +
    ...
    getWeaponSerialNumbers = async (weaponModelName) => {
        const {t} = this.props;
        return await getFetch("/app/weapon/get-active-weapons-by-model-name?modelName=" + weaponModelName)
            .then(response => {
                if (response.ok) {
                    return response;
                } else if (response.status === 400) {
            ...
            .then(data => {
                if (data.length === 0) {
                    this.props.displaySnackbar(SnackBarType.error, t("error.noAvailableWeapons"));
                }
                const weaponSerialNumbers = data.map(item => {
                    return {
                ...
                this.setState({
                    weaponSerialNumbers: weaponSerialNumbers,
                });
                return weaponSerialNumbers;
            }).catch(() => {
                this.props.displaySnackbar(SnackBarType.error, t("common.requestError"));
            })
    ...
    handleChangeWeaponModelName = (event) => {
        this.getWeaponSerialNumbers(event.target.value).then((weaponSerialNumbers) =>
            this.setState({
                weaponModelName: event.target.value,
                weaponSerialNumber: weaponSerialNumbers[0].value
            }, () => this.getConflictReservations())
        )
                ...
                <CardContent>
                    <form onSubmit={this.handleSubmit}
                        ...
                        <SelectFieldComponent
                            ...
                            onChange={this.handleChangeWeaponModelName}
                            name="weaponModelName"
                            items={this.state.weaponModelNames}/>
                        <SelectFieldComponent
                            labelId="select-weapon-serial-number"
                            label={t("reservationDetails.weaponSerialNumber")}
                            value={this.state.weaponSerialNumber}
                            onChange={this.handleChange}
                            name="weaponSerialNumber"
                            items={this.state.weaponSerialNumbers}/>
                        <TextField
                            id="date"
...
export default withTranslation()(withRouter(EditReservationComponent));
