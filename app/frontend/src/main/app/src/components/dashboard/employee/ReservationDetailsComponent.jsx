...
import EditReservationComponent from "./EditReservationComponent";
...
class ReservationDetailsComponent extends Component {
    ...
    loadData = () => {
        const {t} = this.props;
        getFetch('/app/reservation/get-reservation?reservationNumber=' + this.props.match.params.reservationNumber)
            .then((response) => {
                if (response.ok) {
                    return response;
    ...
    render() {
        ...
        if (this.state.editEnabled) {
            return (<EditReservationComponent handleDisplayEdit={this.handleDisplayEdit}
                        ...
                        <Grid container justify="center" spacing={3}>
                            ...
                            <Grid item xs={12}
                                ...
                                <TextFieldComponent onChange={this.handleChange}
                                                    ...
                                                    value={this.state.reservation ? this.state.reservation.customer.name : t("common.loading")}
                                                    name="customerLogin"
                                                    disabled
                                />
                                <TextFieldComponent onChange={this.handleChange}
                                                    label={t("reservationDetails.customerSurname")}
                                                    value={this.state.reservation ? this.state.reservation.customer.surname : t("common.loading")}
                                                    name="customerLogin"
                                                    disabled
                                />
                                <TextFieldComponent onChange={this.handleChange}
                                                    label={t("reservationDetails.customerLogin")}
                                                    value={this.state.reservation ? this.state.reservation.customer.login : t("common.loading")}
                                                    name="customerLogin"
                                                    disabled
                                />
                                <TextFieldComponent onChange={this.handleChange}
                                                    label={t("reservationDetails.customerEmail")}
                                                    value={this.state.reservation ? this.state.reservation.customer.email : t("common.loading")}
                                                    name="customerEmail"
                                                    disabled
                                />
                                <TextFieldComponent onChange={this.handleChange}
                                                    label={t("reservationDetails.customerPhoneNumber")}
                                                    value={this.state.reservation ? this.state.reservation.customer.phoneNumber : t("common.loading")}
                            ...
                            <Grid item
                                ...
                                <hr/>
                                <TextFieldComponent onChange={this.handleChange}
                                                    label={t("reservationDetails.weaponModelAndSerialNumber")}
                                                    value={this.state.reservation ? this.state.reservation.weapon.weaponModelName + " | " +
                                                        this.state.reservation.weapon.serialNumber : t("common.loading")}
